...
def main(dataset_path, print_flag=True):
    ...
    def train_classifier(df, labels, label_term_dict, label_to_index, index_to_label, dataset_path):
        ...
        model.summary()
        model.compile(loss="binary_crossentropy", optimizer='adam', metrics=[TopKCategoricalAccuracy(k=1),TopKCategoricalAccuracy(k=2),TopKCategoricalAccuracy(k=3)])
        ...
        for i in range(1, topn + 1):
            est_top = np.array([[label_to_index[label] for label in pred[:i]] for pred in predictions])
            ...
            perf_summary = perf_summary.append({'model': 'model', 'metric': 'top%d' % i, 'value': perf.mean()}, ignore_index=True)
